...
router.post("/addstepentry", authTokenHandler, async (req, res) => {
  ...
  if (!date || !steps) {
    return res
      .status(400)
      .json(createResponse(false, "Please provide date and steps count"));
  }
...
router.post("/getstepsbydate", authTokenHandler, async (req, res) => {
  ...
  if (!date) {
    ...
    return res.json(
      createResponse(true, "Steps entries for today", user.steps)
    );
  }
...
router.post("/getstepsbylimit", authTokenHandler, async (req, res) => {
  ...
  if (!limit) {
    ...
    return res.json(
      createResponse(
        true,
        `Steps entries for the last ${limit} days`,
        user.steps
      )
    );
  }
